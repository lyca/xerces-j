...
if not exist "%WRAPPER_JAR%" (
    ...
    for /f "tokens=*" %%i in ('certutil -hashfile "%WRAPPER_JAR%" SHA256 ^| findstr /v ":" ^| findstr /v "CertUtil"') do set CHECKSUM_FOUND=%%i
    set CHECKSUM_FOUND=!CHECKSUM_FOUND: =!
    if /I not "!CHECKSUM_FOUND!"=="%WRAPPER_SHA256%" (
        echo ERROR: SHA-256 checksum mismatch for gradle-wrapper.jar! 1>&2
        del "%WRAPPER_JAR%"
        goto fail
    )
)
