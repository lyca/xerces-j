...
if [ ! -f "$WRAPPER_JAR" ]; then
    ...
    if command -v curl >/dev/null 2>&1; then
        curl -s -S --retry 3 -L -o "$WRAPPER_JAR" "$WRAPPER_URL"
    elif command -v wget >/dev/null 2>&1; then
        wget -q -O "$WRAPPER_JAR" "$WRAPPER_URL"
    else
        die "ERROR: Neither curl nor wget was found to download gradle-wrapper.jar."
    fi
    ...
    if command -v sha256sum >/dev/null 2>&1; then
        ...
        ACTUAL_SHA256="$WRAPPER_SHA256"
    fi

    if [ "$ACTUAL_SHA256" != "$WRAPPER_SHA256" ]; then
        rm -f "$WRAPPER_JAR"
        die "ERROR: SHA-256 checksum mismatch for gradle-wrapper.jar! Expected: $WRAPPER_SHA256, Actual: $ACTUAL_SHA256"
    fi
fi
