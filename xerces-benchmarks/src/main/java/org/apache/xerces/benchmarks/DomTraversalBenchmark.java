...
public class DomTraversalBenchmark {
    ...
    private void recursiveTraverse(Node node, Blackhole bh) {
        bh.consume(node.getNodeType());
        bh.consume(node.getNodeName());
        String val = node.getNodeValue();
        if (val != null) {
            bh.consume(val.length());
        }
        Node child = node.getFirstChild();
        while (child != null) {
            recursiveTraverse(child, bh);
            child = child.getNextSibling();
        }
    }

    private void treeWalkerTraverse(Document doc, Blackhole bh) {
        if (doc instanceof DocumentTraversal) {
            TreeWalker walker = ((DocumentTraversal) doc).createTreeWalker(doc, NodeFilter.SHOW_ALL, null, true);
            Node n = walker.getRoot();
            while (n != null) {
                bh.consume(n.getNodeType());
                bh.consume(n.getNodeName());
                n = walker.nextNode();
            }
        }
    }

    private void nodeIteratorTraverse(Document doc, Blackhole bh) {
        if (doc instanceof DocumentTraversal) {
    ...
    @Benchmark
    public void traverseRecursiveXercesDeferred(Blackhole bh) throws Exception {
        ...
        recursiveTraverse(doc, bh);
    }

    @Benchmark
    public void traverseRecursiveXercesEager(Blackhole bh) throws Exception {
    ...
    @Benchmark
    public void traverseRecursiveJdkDeferred(Blackhole bh) throws Exception {
    ...
    @Benchmark
    public void traverseTreeWalkerXercesDeferred(Blackhole bh) throws Exception {
        DocumentBuilder builder = xercesDeferredFactory.newDocumentBuilder();
        Document doc = builder.parse(new ByteArrayInputStream(xmlBytes));
        treeWalkerTraverse(doc, bh);
    }

    @Benchmark
    public void traverseNodeIteratorXercesDeferred(Blackhole bh) throws Exception {
        DocumentBuilder builder = xercesDeferredFactory.newDocumentBuilder();
        Document doc = builder.parse(new ByteArrayInputStream(xmlBytes));
        nodeIteratorTraverse(doc, bh);
    }
}
